...
//     hasError = false;
//
//     final validator = widget.validator ?? (val) {
//       if (val == null || val.isEmpty) return "Required";
//       if (widget.isEmail == true &&
//           !AppConstants.emailValidator.hasMatch(val)) {
...
//
//       if (valid != isValid || error != hasError) {
//         setState(() {
//           isValid = valid;
//           hasError = error;
//         });
...
//     // WidgetsBinding.instance.addPostFrameCallback((_) {
//     //   final text = widget.controller.text.trim();
//     //   final error = widget.validator?.call(text) != null;
//     //
//     //   setState(() {
...
//                 if (mounted) {
//                   setState(() {
//                     hasError = result != null;// 👉 update error flag
//                     isValid = value != null && value.trim().isNotEmpty;
//                   });
//                 }
//               });
//
...
class _CustomTextFieldedState extends State<CustomTextFielded> {
  ...
  void initState() {
    ...
    _hasError = error && text.isNotEmpty; // Only show error if user has entered something
    ...
    widget.controller.addListener(() {
      ...
      if (valid != _isValid || error != _hasError) {
        setState(() {
          _isValid = valid;
          _hasError = error; // Remove the condition here - always update error state
        });
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    final bool showShadow = !_touched && !_hasError;

    // Background color logic: Always transparent when touched or has error
    final Color backgroundColor = (_touched || _hasError)
        ? Colors.transparent
